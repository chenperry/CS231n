...
def discriminator_loss(logits_real, logits_fake):
    ...
    left = bce_loss(logits_real, torch.ones_like(logits_real))
    right = bce_loss(logits_fake, torch.zeros_like(logits_fake))
    loss = left + right
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    left = (scores_real - 1) ** 2 / (2 * N)
    right = scores_fake ** 2 / (2 * N)
    loss = (left + right).sum()
